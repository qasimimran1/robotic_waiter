...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        try:
            (trans,rot) = tf_target_left.lookupTransform('/base_footprint', '/left', rospy.Time(0))
            (trans_r,rot_r) = tf_target_left.lookupTransform('/base_footprint', '/right', rospy.Time(0))            
...
# endEffector_rot [0.4999999999966269, -0.4999999999966269, -0.5000018366025517, 0.49999816339744835]
            ...
            pose_target_l.orientation.x = -0.713121567362           
            pose_target_l.orientation.y = -0.00121925961248         
            pose_target_l.orientation.z = -0.701036901706                    
            pose_target_l.orientation.w = 0.0018455390891
      ...
      # w: 0.00185401210989
            ...
            target_position = trans
            target_position[0] = trans[0] - 0.08
            target_position[1] = trans[1]
            target_position[2] = trans[2] + 0.002
            
            # left_arm_group.set_pose_target(pose_target_l)            
            left_arm_group.set_position_target(trans )
