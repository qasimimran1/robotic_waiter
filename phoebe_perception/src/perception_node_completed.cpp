...
int main(int argc, char *argv[])
{
  ...
  camera_frame = priv_nh_.param<std::string>("camera_frame", "xtion_link");
  voxel_leaf_size = priv_nh_.param<float>("voxel_leaf_size", 0.002);
  x_filter_min = priv_nh_.param<float>("x_filter_min", -3.5);
  x_filter_max = priv_nh_.param<float>("x_filter_max",  3.5);
  y_filter_min = priv_nh_.param<float>("y_filter_min", -3.5);
  y_filter_max = priv_nh_.param<float>("y_filter_max",  3.5);
  z_filter_min = priv_nh_.param<float>("z_filter_min", -3.5);
...
//               ros::topic::waitForMessage<sensor_msgs::PointCloud2>(topic, nh);
  ...
  ROS_INFO_STREAM("PASSTHROUGH FILTER(S) Done");
