...
int main(int argc, char *argv[])
{
  ...
  cloud_topic = priv_nh_.param<std::string>("cloud_topic", "/xtion/depth/points");
  world_frame = priv_nh_.param<std::string>("world_frame", "base_footprint");
  camera_frame = priv_nh_.param<std::string>("camera_frame", "xtion_optical_frame");
  voxel_leaf_size = priv_nh_.param<float>("voxel_leaf_size", 0.01);
  x_filter_min = priv_nh_.param<float>("x_filter_min", -1.0);
  x_filter_max = priv_nh_.param<float>("x_filter_max",  3.5);
  y_filter_min = priv_nh_.param<float>("y_filter_min", -1.0);
  y_filter_max = priv_nh_.param<float>("y_filter_max",  1.0);
  z_filter_min = priv_nh_.param<float>("z_filter_min", 0.1);
  z_filter_max = priv_nh_.param<float>("z_filter_max",  3.5);
  plane_max_iter = priv_nh_.param<int>("plane_max_iterations", 200);
  plane_dist_thresh = priv_nh_.param<float>("plane_distance_threshold", 0.02);
  cluster_tol = priv_nh_.param<float>("cluster_tolerance", 0.025);
  cluster_min_size = priv_nh_.param<int>("cluster_min_size", 200);
 ...
 while (ros::ok())
 {
  ...
  try
  {
    listener.waitForTransform(world_frame, recent_cloud->header.frame_id,  ros::Time::now(), ros::Duration(2.0));
...
//               ros::topic::waitForMessage<sensor_msgs::PointCloud2>(topic, nh);
  ...
  * ========================================*/
  ...
  kdtree.setInputCloud (sor_cloud_filtered);
...
/*   if(pcl::computeCentroid(*sor_cloud_filtered, centroid) > 0)
{
	
	if ( kdtree.nearestKSearch (right, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 )
  {
	  // computePointNormal (const pcl::PointCloud<PointInT> &cloud, const std::vector<int> &indices, Eigen::Vector4f &plane_parameters, float &curvature);   (pcl::PointCloud<pcl::PointXYZ>)
   computePointNormal(*sor_cloud_filtered, pointIdxNKNSearch, plane_parameters, curvature);
   //ROS_INFO_STREAM("Curvature: " << curvature<< "." );   
   ROS_INFO_STREAM("xn:  " << plane_parameters[0]<<   "  yn:  " << plane_parameters[1]<<  "  zn:  " << plane_parameters[2]<<"." );	  
	  
  }
    // define orientation which will be sent to robot
    orientation = atan2 (centroid.y, centroid.x);	
	ROS_INFO_STREAM("Centroid x: " << centroid.x << " y: " << centroid.y << " z: " << centroid.z << " with orientation " << orientation << "." );
} */
 ...
 // center_transform.setOrigin( tf::Vector3(sor_cloud_filtered->at(1).x, sor_cloud_filtered->at(1).y, sor_cloud_filtered->at(1).z) );
  center_transform.setOrigin( tf::Vector3(centroid.x,  centroid.y,   centroid.z) );
  
  q.setRPY(0, 0, 0);
  center_transform.setRotation(q);
  br.sendTransform(tf::StampedTransform(center_transform, ros::Time::now(), world_frame, "center")); */
  ...
  pcl::toROSMsg(  *sor_cloud_filtered , *pc2_cloud);  //  zf_cloud    *(clusters.at(0))   *cloud_f  *sor_cloud_filtered
